(()=>{
    ...
    // ENDPOINTS    
    const endPoints = ['/resize_w/800/resize_h/600', '/crop_w/500/crop_h/500/fp/.65,.37', '/focalpointsf-py=45%/', 'p-x=45%/', '/scale(1.2)', '/filter/Grayscale', '/hsb_h/-1.0/hsb_s/0.0/hsb_b/1.0',''];
    ...
    const showFocalLeft = ()=>{
        ...
        setTimeout(()=>imgContainer.classList.add('moveLeft'), 1000);
    }

    const showFocalBottom = ()=>{
        imgContainer.classList.add('showBottom');
        setTimeout(()=>imgContainer.classList.add('moveBottom'), 1000);
    }
